...
function bodyParts(creepRole) {
  ...
  for (var roleName in creepRole) {
    ...
    if (roleName === 'upgrader' || roleName === 'builder') {
      if (energyLevel > 400) {
        var workEnergy = energyLevel - 300;
        creepRole[roleName].body.push(WORK,WORK,WORK);
      }
      else {
        var workEnergy = energyLevel - 200;
        creepRole[roleName].body.push(WORK,WORK);
      }  
      var numBodyParts = workEnergy / 50;
      var numCarryParts = Math.ceil(numBodyParts / 2);
      var numMoveParts = Math.trunc(numBodyParts - numCarryParts);
      for (var i = 0; i < numCarryParts; i++) {
        creepRole[roleName].body.push(CARRY);
